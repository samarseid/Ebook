...
const Reader = () => {
  ...
  const bookId = 'atomic-habits.pdf';
...
useEffect(() => {
  ...
  useEffect(() => {
    tg.ready();
    loadFormattedPdfPages('/books/atomic-habits.pdf')
      .then((loadedPages) => {
    ...
    >
      ...
      {currentPage === 0 ? (
        <div style={{ textAlign:'center', marginBottom:'1rem' }}>
          <h1 style={{ fontSize:'32px', fontWeight:'bold', marginBottom:4, fontFamily }}>Atomic Habbits</h1>
          <h2 style={{ fontSize:'18px', color:textMuted, fontFamily }}>1 Chapter</h2>
        </div>
      ) : (
        <div style={{ textAlign:'center', fontWeight:600, marginBottom:'1rem', fontSize:'16px', color:textMuted, fontFamily }}>
          {Math.floor(currentPage / 10) + 1} Chapter
        </div>
      )}
